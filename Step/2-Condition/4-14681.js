...
const fs = require('fs');
const [A, B] = fs.readFileSync('./stdin/4-14681.txt').toString().trim().split('\n');

if(A < -1000 || A > 1000 || A == 0 || B < -1000 || B > 1000 || B == 0) console.log('Wrong inputs');
else if(A > 0 && B > 0) console.log(1);
...
// ★ 삼항연산자 이용
// const fs = require('fs');
// const [x, y] = fs.readFileSync('./stdin/4-14681.txt').toString().trim().split('\n').map(Number);
